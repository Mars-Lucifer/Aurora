...
> [!warning]
> Разработка Авроры прекращена, в данный момент не планируется продолжения разработки :(
...
После выполнения этих действий вы можете запустить ассистента запустив файл `start.bat`

# Документация

Аврора - это приложение с модульной архитектурой, частично основанной на микросервисах. Она состоит из нескольких модулей, каждый из которых может быть написан на разных языках программирования. Эти модули выполняют различные функции и обмениваются информацией друг с другом, используя сокеты. Ниже пойдёт информация именно про запросы и сокеты.
...
async function main() {
    const result = await connect({ message: { recipient: ["Client 1", "Client 3"], sender: "Client 2", content: "Hello" }, type: "active" });
    console.log(result);
}
...
async def main():
    ...
    result = await connect(recipient="Client 1", sender="Client 2", content="Hello", type="active")
    print(result)
...
> [!TIP]
> Функции запросов через JS действуют намного быстрее чем запросы через Python
...
Запросы отправляются и принимаются в форме JSON строк. Пример представления запроса:

```javascript
// Клиент который отправляет запрос
await connect(recipient=["Client 1", "Client 3"], sender="Client 2", content=["Hello", "more info"], type="active");
```

```javascript
// Клиент который принял запрос
const request = {
  "recipient": ["Client 1", "Client 3"],
  "sender": "Client 2",
  "content": ["Hello", "more info"]
};
```

Вот что означает каждая часть запроса:
- **recipient** - получатель, может быть как один получатель, так и несколько
- **sender** - отправитель, по логике должен быть только один
- **content** - само сообщение, может быть массивом и содержатть несколько элементов, если того требует модуль (к примеру, модуль sinthesis)
...
- [**Voice synthesis (синтез речи)**](#voice-synthesis): Этот модуль отвечает за синтез речи Авроры.
...
Модуль отвечающий за синтез речи Авроры. Для его полного функционирования требуется подключить ключ API от [elevenlabs.io](https://elevenlabs.io/), вы можете получить его бесплатно на их сайте, с лимитом 10,000 символов в месяц. Сейчас требуется в главной папке Авроры создать файл `key.txt` и вставить туда ключ, после этого вам будет доступен этот модуль (в будущем изменю эту систему, будет использован GUI).

Модуль синтеза имеет две основные функции:
- **Первичный синтез** - вызывается для повторяющихся запросов. То есть он используется для синтеза тех фраз которые Аврора говорит чаще всего и их можно заранее записать. По сути это является лишь воспроизведением Аудио файлов записанных заранее. Это нужно для экономии запросов по API. Для обращения к этому синтезу требуется выполнить запрос вида
- ```json
  {
    "recipient": "sinthesis",
    "sender": "your client",
  ...
  ```
  - **sinthesis** - клиент к которому вы обращаетесь, то есть клиент синтеза
  - **you client** - ваш клиент
  - 0 - шаблон который вы хотите использовать. Имеются:
    - 0 - шаблон активации. Пример: "Аврора" -> "Слушаю вас"
    - 1 - шаблон выполнения. Пример: "Аврора, запусти браузер" -> "Выполняю"
    - 2 - шаблон звука запуска приложения
  - 0 - вид синтеза который вы хотите использовать. `0` - первичный синтез, `1` - вторичный синтез
- **Вторичный синтез** - используется для не повторяющихся запросов. То есть, если никакой шаблон не подходит под запрос, используется этот синтех который может синтезирвоать любой текст с помощью API. Вид запроса:
- ```json
  {
    "recipient": "sinthesis",
    "sender": "your client",
    "content": ["Ваше любое сообщение", "1"]
  }
  ```
