...
async def main():
    ...
    print(await connect(message="Client 2%_Client 1%_Hello", type="active"))
...
const request = 'Client 1%_Client 2%_Message%_more';
```
...
```javascript
// Клиент который отправляет запрос
const request = ['Client 1', 'Client 2', 'Message', 'more'];
```
...
- Четвертый аргумент (дополнительный) - дополнение для некоторых модулей, например модуля синтеза речи, `more`
...
Модуль отвечающий за синтез речи Авроры. Для его полного функционирования требуется подключить ключ API от elevenlabs.io, вы можете получить его бесплатно на их сайте, с лимитом 10,000 символов в месяц. Сейчас требуется в главной папке Авроры создать файл `key.txt` и вставить туда ключ, после этого вам будет доступен этот модуль (в будущем изменю эту систему, будет использован GUI).

Модуль синтеза имеет две основные функции:
- **Первичный синтез** - вызывается для повторяющихся запросов. То есть он используется для синтеза тех фраз которые Аврора говорит чаще всего и их можно заранее записать. По сути это является лишь воспроизведением Аудио файлов записанных заранее. Это нужно для экономии запросов по API. Для обращения к этому синтезу требуется выполнить запрос вида `['sinthesis', 'you client', '0', '0']` (запрос уже переведён в массив для облегчения читаемости), где:
...
- **Вторичный синтез** - используется для не повторяющихся запросов. То есть, если никакой шаблон не подходит под запрос, используется этот синтех который может синтезирвоать любой текст с помощью API. Вид запроса: `['sinthesis', 'you client', 'Message', '1']`
